    ...
    Public Sub Save()
        ...
        If My.Settings.SaveDebug Then
            ...
            Try
                ...
                If errors.Count > 0 Then sw.WriteLine(vbCrLf & String.Join(vbCrLf, errors))
                sw.Close()
                fs.Close()
            Catch e As Exception
                Console.Write(String.Join(vbCrLf, debug))
                If errors.Count > 0 Then sw.WriteLine(vbCrLf & String.Join(vbCrLf, errors))
                Console.Write("! System Error(7): Invalid log filename '" + fichero + "'. " + e.Message)
                Console.ReadLine()
            End Try
        Else
            Console.Write(String.Join(vbCrLf, debug))
            If errors.Count > 0 Then Console.WriteLine(vbCrLf & String.Join(vbCrLf, errors))
        End If
    End Sub
End Class
