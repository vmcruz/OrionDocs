...
function writemenu(menuItems) {
	...
	var html = createMenu(mergeItems(menuItems), {
		...
		item: {
			class: 'fancy-method-item',
			parentclass: 'fancy-class-list',
			currentclass: 'fancy-method-selected',
		...
		if(obj.hasOwnProperty(key)) {
			if(isObject(obj[key])) {
				...
				if(obj[key].hasOwnProperty(0) && obj[key][0].length > 0)
					htmlMenu += '<li class="' + config.item.parentclass + '"><span class="' + selectedClass + '"' + dataInfo + '>' + key + '</span>' + createMenu(obj[key], config) + '</li>';
				else
				...
				for(var i = 0; i < obj[key].length; i++) {
					...
					if(!obj.hasOwnProperty(data[0])) {
						...
						htmlMenu += '<li class="' + config.item.class + '-' + exposure + selectedClass + '" data-url="' + data[1] + '.' + data[2] + '" data-item="' + dataItem + '"><span>' + data[0] + '</span>' + "</li>";
					}
				}
			}
		}
